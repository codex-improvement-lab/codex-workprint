...
test("the Intake to Proofline CLI projection preserves producer semantics without copying private bindings", async () => {
  // Captured from the explicitly synthetic 17-operation producer loop on Windows.
  const input = JSON.parse(await readFile(new URL("./fixtures/proofline-intake-mainline.json", import.meta.url), "utf8"));
